...
std::map<int, int> PC::THLScanImpl(const unsigned short chip,
				   const int coarse,
    ...
    // Implementation of the actual THL scan, which sets all other chips besides the
    // current active chip to THL 500 (to avoid the issue of chips, which have a
    // broken DAC for values > 512) and scans the range given with the given shutter
    // range and mode
    // Returns: a map of the registered THL / Hit pairs, to calculate the location of the
    // THL value from
    std::map<int, int> thlMap;
    for(auto chip: _chip_set){
        thlMap.insert(std::pair<int, int>(chip, fpga->tp->GetDAC(6, chip)));
    ...
    std::string filename;
    filename = "data/THLScan_" + std::to_string(chip) + ".txt";
    thlStream.open(filename, std::fstream::app);

    thlStream << "# Date: " << currentDateTime() << std::endl;
    thlStream << "# THL\t Hits\t CoarseTHL" << std::endl;
    ...
    const int lower_thl_bound = threshold_boundaries.first;
    const int upper_thl_bound = threshold_boundaries.second;
    int thl = lower_thl_bound;

    // define array to store pixel data after each readout in. Will be initialized in the
    // SaveData calls, which is why it's defined here and not in the do-while loop
    FrameArray<int> pixel_data;

    // map storing THL values versus hits at that value to determine threshold
    std::map<int, int> countMap;

    do{
        // write threshold and read take data with given shutter time
        fpga->tp->SetDAC(6, chip, thl);
        fpga->WriteReadFSR();
        usleep(10000);
        // was 255, 0 before
        fpga->CountingTime(shutter_time, shutter_range);
        const int result = fpga->DataChipFPGA();
        if (result != 0){
	...
	Frame hitFrame = Frame(pixel_data);
	hitFrame.CalcFullFrameVars(0);
	const int hits = hitFrame.GetFullFrameHits();
	
        // insert into map
        countMap.insert(std::pair<int, int>(thl, hits));
    ...
    for(auto chip: _chip_set){
        fpga->tp->SetDAC(6, chip, thlMap[chip]);
    }
...
int PC::CalculateThreshold(std::map<int, int> countMap){
    // calculates the position of the 0 plateau for an artificial "noise
    // SCurve", given by a THL scan over the whole chip, read out using
    // zero suppression.
    // countMap contains THL / count pairs, which will be 4096 for low
    // THL values and at some point drop to 0 within a few THL values.
    ...
    // else we simply return 0, because we did not find 5 consecutive 0 values
    return 0;
}

int PC::DoTHLScan(const unsigned short chip,
		  const std::pair<int, int> threshold_boundaries,
		  const std::string shutter_range,
		  const std::string shutter_time,
		  std::atomic_bool *loop_stop){
    // THL scan, which tries to find the THL of the given chip using a smart
    // approach
    // - all other chips are set to THL == 500
    // - scan THL range given by the user with short shutter range 
    //   (irregardless of user setting always (1, 100).
    // - determine location of 0 plateau, start long scan around that point +- 30 THL values
    // - determine location of 0 plateau again -> THL value

    std::map<int, int> countMap;
    // do first scan
    const int coarse = 7;
    const std::string shutterRangeStart = "l";
    const std::string shutterTimeStart = "100";
    countMap = THLScanImpl(chip,
			   coarse,
			   threshold_boundaries,
			   shutterRangeStart,
			   shutterTimeStart,
			   loop_stop);
    // calculate location of plateau
    const int roughThreshold = CalculateThreshold(countMap);
    if (roughThreshold > 0){
	// start fine scan based on rough threshold - 35 and plus 25, to cover
	// +- 30 roughly from real noise end
	const std::pair<int, int> fineThlBounds(roughThreshold - 35, roughThreshold + 25);
	countMap = THLScanImpl(chip,
			       coarse,
			       fineThlBounds,
			       shutter_range,
			       shutter_time,
			       loop_stop);
	// calculate threshold based on fine scan and write result to stdout and file
	const int threshold = CalculateThreshold(countMap);
	// append to file
	std::fstream thlStream;
	std::string filename;
	filename = "data/THLScan_" + std::to_string(chip) + ".txt";
	...
	std::cout << "# Finished Thl scan with THL of: " << threshold << std::endl;
	thlStream << "# Finished Thl scan with THL of: " << threshold << std::endl;
	thlStream.close();
    }
    else{
	std::cout << "Could not find a rough threshold for chip "
		  << chip << "!" << std::endl;
	return -1;
    }
...
int PC::DoRawTHLScan(const unsigned short chip,
                     const std::pair<int, int> coarse_boundaries,
                     const std::pair<int, int> threshold_boundaries,
                     const std::string shutter_range,
                     const std::string shutter_time,
        	     std::atomic_bool *loop_stop){
    // simple THL scan, which simply runs over the THL values once with the
    // given settings, writes to file and stdout
	...
	THLScanImpl(chip, coarse, threshold_boundaries, shutter_range, shutter_time, loop_stop);
    }
